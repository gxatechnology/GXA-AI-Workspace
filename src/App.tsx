...
export default function App() {
  ...
  const [authMode, setAuthMode] = useState<'login' | 'register' | null>(() => typeof window !== 'undefined' && new URLSearchParams(window.location.search).has('auth') ? 'login' : null);
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/profile', { headers: authHeaders(restoredUser), credentials: 'same-origin' })
        .then(async response => { if (!response.ok) throw new Error('Session expired.'); return response.json(); })
        .then(async body => { const next = { ...body.user, sessionToken: 'cookie-session' }; setCurrentUser(next); localStorage.setItem('gxa_user', JSON.stringify(next)); const current = await fetchCurrentPlan(next).catch(() => null); setCurrentPlanKey(current?.currentPlanKey || next.subscription || 'free'); })
        .catch(() => { localStorage.removeItem('gxa_user'); setCurrentUser(guestUser); })
        .finally(() => setAuthReady(true));
  }, []);

  useEffect(() => { const limited = () => setPlanLimitOpen(true); const updated = (event: Event) => { const user = (event as CustomEvent).detail; if (user) setCurrentUser(user); }; window.addEventListener('gxa:plan-limit', limited); window.addEventListener('gxa:profile-updated', updated); return () => { window.removeEventListener('gxa:plan-limit', limited); window.removeEventListener('gxa:profile-updated', updated); }; }, []);
  ...
  return <div className="app-shell flex h-screen overflow-hidden font-sans" data-theme={theme}>
    ...
    <PlanLimitDialog open={planLimitOpen} onClose={() => setPlanLimitOpen(false)} onUpgrade={() => { setPlanLimitOpen(false); selectWorkspace('pricing'); }} />
  </div>;
}
