    ...
    <div className="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 space-y-4">
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950 sm:p-5">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><h2 className="text-lg font-black">{tool.name}</h2>{locked(tool) && <Lock className="h-4 w-4 text-amber-500" />}</div><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-500">{tool.description}</p></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold dark:bg-zinc-900">{tool.requiredPlan === 'pro_plus' ? 'Pro' : tool.requiredPlan === 'pro' ? 'Starter' : 'Free'}</span></div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950 sm:p-5">
          ...
          {tool.operation === 'generate' && <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><label className="text-xs font-bold">Aspect ratio<select value={aspectRatio} onChange={(event) => setAspectRatio(event.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs dark:border-zinc-800 dark:bg-zinc-900">{config.aspectRatios.map((item) => <option key={item}>{item}</option>)}</select></label><label className="text-xs font-bold">Quality<select value={quality} onChange={(event) => { if (event.target.value === '4K' && config.currentPlan !== 'pro_plus') { onOpenUpgradeModal(); return; } setQuality(event.target.value); }} className="mt-1.5 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs dark:border-zinc-800 dark:bg-zinc-900">{config.qualities.map((item) => <option key={item}>{item}{item === '4K' ? ' · Pro' : ''}</option>)}</select></label><label className="text-xs font-bold">Style<select value={style} onChange={(event) => setStyle(event.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs dark:border-zinc-800 dark:bg-zinc-900">{config.styles.map((item) => <option key={item}>{item}</option>)}</select></label><label className="text-xs font-bold">Batch<select value={batch} onChange={(event) => { const value = Number(event.target.value); if (value > 1 && config.currentPlan === 'free') { onOpenUpgradeModal(); return; } setBatch(value); }} className="mt-1.5 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs dark:border-zinc-800 dark:bg-zinc-900">{Array.from({ length: Math.max(1, config.limits.batch) }, (_, index) => index + 1).map((item) => <option key={item} value={item}>{item}{item > 1 ? ' · Pro' : ''}</option>)}</select></label></div>}
